...
struct frame* create_frame(struct c63_common *cm, yuv_t *image)
{
	...
	f->mbs[Y] = calloc(cm->mb_rows[Y] * cm->mb_cols[Y], sizeof(struct macroblock));
	f->mbs[U] = calloc(cm->mb_rows[U] * cm->mb_cols[U], sizeof(struct macroblock));
	f->mbs[V] = calloc(cm->mb_rows[V] * cm->mb_cols[V], sizeof(struct macroblock));
